...
$conteosArbolCategorias = $db->getConteosArbolCategorias();
...
/**
 * Incluye subcategorías descendientes al filtrar (p. ej. nivel 2 incluye nivel 3 oculto en UI).
 */
function expandCategoryFilter(array $selected, array $categorias): array
{
    if ($selected === []) {
        return [];
    }
    ...
    foreach ($categorias as $cat) {
        $byName[$cat['nombre']] = $cat;
        if (!empty($cat['parent_id'])) {
            $childrenByParent[(int)$cat['parent_id']][] = $cat;
        }
    }
    ...
    foreach ($selected as $name) {
        ...
        while ($stack !== []) {
            ...
            $expanded[] = $current['nombre'];
            foreach ($childrenByParent[(int)$current['id']] ?? [] as $child) {
                $stack[] = $child;
            }
        }
    }
...
$cat_filter_db = expandCategoryFilter($cat_filter, $categoriasConConteo);
              ...
              <div class="categoria-nivel-1">
                ...
                <div class="flex items-center gap-1">
                  ...
                  <label class="flex flex-1 cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-xs font-semibold text-navy transition hover:bg-[#f4f8fb]">
                    ...
                    <span class="rounded-full bg-blue/10 px-2 py-0.5 text-[10px] font-semibold text-blue"><?= (int)($conteosArbolCategorias[(int)$cat1['id']] ?? 0) ?></span>
                  </label>
                ...
                <?php if ($hasChildren): ?>
                  <div id="cat-<?= $cat1Id ?>" class="categoria-content ml-4 mt-1 space-y-1 <?= $isExpanded ? '' : 'hidden' ?>">
                    <?php foreach ($subCats as $cat2):
                      ...
                      $count2 = (int)($conteosArbolCategorias[(int)$cat2['id']] ?? 0);
                    ?>
        ...
        <div id="product-grid" class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          <?php foreach ($page_items as $p): ?>
            ...
            >
              ...
              <div class="product-card-media relative h-44 w-full shrink-0 overflow-hidden">
                <img
              ...
              <!-- Cuerpo card -->
              <div class="product-card-body flex flex-1 flex-col gap-3 p-5">
                <div class="product-card-info">
                  ...
                  <h3 class="mt-1 font-semibold leading-snug text-navy">
                    <?= htmlspecialchars($p['nombre'], ENT_QUOTES, 'UTF-8') ?>
                  </h3>
                </div>

                <!-- Features -->
                <ul class="product-card-features space-y-1">
                ...
                <!-- Acciones -->
                <div class="product-card-actions mt-auto flex flex-col gap-2 pt-3">
